...
class KTX2Loader extends CompressedTextureLoader {
	...
	initModule() {
		...
		scope.basisModulePending = new Promise( function ( resolve, reject ) {

			var createTranscoder = globalThis.MSC_TRANSCODER;
			if ( ! createTranscoder ) {

				reject( new Error( 'THREE.KTX2Loader: Basis transcoder script is not loaded.' ) );
				return;

			}

			createTranscoder().then( function ( basisModule ) {
